...
float ReLU6(float x, float negative_slope) {
	float res = std::min(std::max(0.0f, x) + negative_slope * std::min(0.0f, x), 6.0f);
	return res;
}
...
int GetConvolutionOutputSize(int N, int k, int pad, int stride, int dilationrate) {
    // 
